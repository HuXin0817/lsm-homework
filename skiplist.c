...
SkipListNode *new_skip_list_node(int key, int height) {
  ...
  SkipListNode *node =
      malloc(sizeof(SkipListNode) + (height - 1) * sizeof(SkipListNode *));
  node->key = key;
  node->height = height;
  return node;
}
...
SkipListNode *find_node(SkipList *skip_list, int key, SkipListNode **prev_nodes,
                        int prev_nodes_height) {
...
SkipListNode *next_skip_list_node(SkipListNode *node) { return node->next[0]; }
...
void fix_max_height(SkipList *skip_list) {
